...
Form.MultipleFileInput = new Class({
	...
	add: function(file){
		if(this.checkFiles.contains(file.name) == false){
			this.checkFiles.push(file.name);
			...
			return this;
		}else{alert("File name already exist");return false};
	...
	remove: function(file){
		...
		var remove = this.checkFiles.indexOf(file.name);
		if (index == -1) return this;
		this._files.splice(index, 1);
		this.checkFiles.splice(remove, 1);
