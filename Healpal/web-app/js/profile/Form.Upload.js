...
Form.Upload = new Class({
	...
	modernUpload: function(input){
		...
		var list = new Element('ul.uploadList').inject(input, 'after');

		//var progress = new Element('div.progress')
			//.setStyle('display', 'none').inject(list, 'after');
		
		var list1= new Element('p',{text: this.options.or}).inject(input, 'before');

		var uploadReq = new Request.File({
			url: form.get('action'),
			//onRequest: progress.setStyles.pass({display: 'block', width: 0}, progress),
			onProgress: function(event){
				  $('#load-img').html('<img src="../assets/image/spinner.gif" width="25" height="25" />');
				  $('#load-msg').html('Please wait. Loading...')
