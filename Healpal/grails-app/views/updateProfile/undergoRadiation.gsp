...
<div class="col-sm-12">
			...
			<g:link controller="updateProfile" action="undergoRadiation"
				params="[ans: 'No']"><h3><span>No</span></h3></g:link></div>
								</div>
								<div class="box-content2">
									<g:if test="${value == 'Scheduled for radiation treatment'}">
		...
		<g:link controller="updateProfile" action="undergoRadiation"
			params="[ans: 'Scheduled for radiation treatment']"><h3><span>Scheduled <br>for <br>radiation <br>treatment</span></h3></g:link></div>
								</div>
							</div>
							</div>
						</div>
...
<script>
	...
	$(document).on(
			...
			function() {
				if ($(this).attr('id') != "") {
					var url = "../UpdateProfile/radiationTreatments?answer="
							+ $(this).attr('id');
